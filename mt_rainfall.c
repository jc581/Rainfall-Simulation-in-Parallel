...
void* first_traverse(void* varg) { 
  /* initilize the local variables from thread argument */
  thr_arg* arg = varg;
  ...
  for (i = startRow; i <= endRow; i++) {
	...
	/***** NEED SYNCRONIZATION AT THIS STEP, WHEN UPDATE trickle MATRIX *****/
...
void* second_traverse(void* varg) {
  /* initilize the local variables from thread argument */
  thr_arg2* arg = varg;
  double** curr = arg->curr;
  double_lock** trickle = arg->trickle;
...
int simulate(double** absorb, struct Frac** fraction, int M, double A, int N) {
  int i;
  int t = 0;
  // flag to denote if there still remains any water at certain points
  int isWet = 0;
  ...
  while (isWet || M > 0) { // each iteration is one time step
    ...
    /* Create threads and assign the work first_traverse() to each thread */
    for (i = 0; i < NUM_THREADS; i++) {      
      ...
      p->N = N;
      p->id = i;
      pthread_create(&threads[i], NULL, first_traverse, (void *)(p));  
    }
    /* Join the threads to complete the frist traverse */
    for (i = 0; i < NUM_THREADS; i++) {    
      pthread_join(threads[i], NULL);  
    }
    /* Create threads and assign the work second_traverse() to each thread */
    for (i = 0; i < NUM_THREADS; i++) {    
      q = malloc(sizeof(*q));
      q->curr = curr;
      q->trickle = trickle;
      ...
      q->id = i;
      q->p_isWet = &isWet;
      pthread_create(&threads[i], NULL, second_traverse, (void *)(q));  
    }
    /* Join the threads to complete the second traverse */
    for (i = 0; i < NUM_THREADS; i++) {    
      pthread_join(threads[i], NULL);  
    }
